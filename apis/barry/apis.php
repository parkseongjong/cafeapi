...
// 'E-CTC','E-KRW' 추가 21.09.02 By.ojt
$coin_list_arr = array('E-TP3', 'E-MC','E-CTC','E-KRW');
$coin_list_arr_low = array('etp3', 'emc','ectc','ekrw');
...
switch($kind) {
	...
	case 'payment':
		...
		if ( empty($userData) ) {
			...
			if ( empty($seller_userData) || $seller_userData['virtual_wallet_address'] != $seller_address ) {
				...
				$ok_json['payment_fee'] = $getTokenFeeVal;
				$ok_json['payment_fee_type'] = $module_name;
			}		
		}
				...
				if ( empty($payData) ) {
					...
					if ( empty($amount) ) {
						...
						if ( $coin_type2 == 'coin' ) {
							...
							//ectc는 소수점을 버리지 않고, 8자리까지 살려서 리턴 한다.
							if($coin_unit == 'ectc'){
								//String return $db->inc 에서 넘버릭 검사 통과
								$amount2 = trim(sprintf('%12.8f', $amount2));
							}
							else{
	...
	case 'getprice':
		...
		if($coin_type == 'E-KRW'){
			...
			$coin = new_coin_price_change_won_allowEctc($coin_type_tmp, $price, $coin_type2); // wallet2/config/new_config
		}
	...
	case 'getprice2':
		...
		if($coin_type == 'E-KRW'){
			//E-KRW 비율... 확인
			$epay_per_coin = (string)1;
			$coinData['value'] = (string)1;
		}
		else{
			list($coin_type2, $coin_unit) = barryapi_coin_type_change($coin_type);
			list($module_name, $module_name2) = barryapi_set_module_name($coin_unit);
		...
		break;
	//2021.09.09 By.OJT 환율 값 전체 리턴
	case 'getpriceAll':

		$ok_json['data'][] = ['type'=>'ex_rate','data'=> false];
		$ok_json['data'][] = ['type'=>'epay_per_coin','data'=> false];

		foreach ($coin_list_arr as $key => $value){
			$coin_type = $value;
			$returnCoinType = barryapi_coin_type_change2($coin_type);
			if($coin_type == 'E-KRW'){
				$epay_per_coin = (string) 1;
				$coinData['value'] = (string) 1;
			}
			...
			if ( empty($coinData) ) {
				...
				$ok_json['data'][0]['data'][] = ['value'=>$coinData['value'],'unit'=>$returnCoinType]; // 1 Coin = ? 원
				$ok_json['data'][1]['data'][] = ['value'=>$epay_per_coin,'unit'=>$returnCoinType]; // 1 Coin = ? E-Pay
			}
		}
